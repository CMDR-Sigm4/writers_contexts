...
2. Run the safety check:
   ...
   ```bash
   node .claude/skills/new_chapter/scripts/new_chapter.js '{}'
   ```

   If `working_draft.md` is not empty, the script exits with code 2. Stop immediately, warn the user, and ask whether to run `save_context` first or whether they explicitly allow clearing the draft. Use `--force` only with explicit permission.
...
5. Run the script, adding `--force` only if explicitly authorized:
   ...
   ```bash
   node .claude/skills/new_chapter/scripts/new_chapter.js '<json>'
   ```

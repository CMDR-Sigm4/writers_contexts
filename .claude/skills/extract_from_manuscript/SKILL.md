...
Read an archived chapter in `07_manuscript/` and extract all structured information needed by the context system. This works like `analyze_chapter`, but the source is an existing manuscript file.
...
Use the same extraction standards as `analyze_chapter`:
...
- Before writing to `working_draft.md`, read it:
  ...
  - If it contains prose, stop and ask whether to replace it, leave it unchanged, or run `save_context` first.
